...
#include "SIMON_pipelined_IP.h"
#include "xil_io.h"
#include "simon_hw.h"
#include "xtime_l.h"

int isExecuted;						// done interrupt trigger
uint32_t *ciphertext_ptr;			// ciphertext
int num_blocks;						// no. of blocks
...
void hw_key_write(uint32_t *key){
	...
	Xil_Out32(XPAR_SIMON_PIPELINED_IP_0_S00_AXI_BASEADDR + SIMON_PIPELINED_IP_S00_AXI_SLV_REG2_OFFSET, key[0]);
	Xil_Out32(XPAR_SIMON_PIPELINED_IP_0_S00_AXI_BASEADDR + SIMON_PIPELINED_IP_S00_AXI_SLV_REG3_OFFSET, key[1]);
...
XStatus hw_init(XScuGic *SIMONIntcInstance){
	...
	XScuGic_SetPriorityTriggerType(SIMONIntcInstance, XPAR_FABRIC_SIMON_PIPELINED_IP_0_DONE_INTR_INTR, 0xA0, 0x03);

	// Register the Interrupt Service Routine (ISR)
	status = XScuGic_Connect(SIMONIntcInstance, XPAR_FABRIC_SIMON_PIPELINED_IP_0_DONE_INTR_INTR, (Xil_ExceptionHandler)hw_interrupt, (void *)SIMONIntcInstance);

	// Enable the relevant GIC interrupt
	XScuGic_Enable(SIMONIntcInstance, XPAR_FABRIC_SIMON_PIPELINED_IP_0_DONE_INTR_INTR);
...
XStatus initiate_encryption(uint32_t *plaintext, uint32_t *ciphertext, int length){
	...
	XTime_GetTime(&start);
	Xil_Out32(XPAR_SIMON_PIPELINED_IP_0_S00_AXI_BASEADDR + SIMON_PIPELINED_IP_S00_AXI_SLV_REG0_OFFSET, (length<<1) | 0x1);

	ciphertext_ptr = ciphertext;
	num_blocks = length;

	// De-assert begin bit
	Xil_Out32(XPAR_SIMON_PIPELINED_IP_0_S00_AXI_BASEADDR + SIMON_PIPELINED_IP_S00_AXI_SLV_REG0_OFFSET, (length<<1) & 0xFFFFFFFE);
